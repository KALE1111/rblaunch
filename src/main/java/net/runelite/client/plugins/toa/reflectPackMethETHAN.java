...
@Singleton
@Slf4j
public class reflectPackMethETHAN implements reflectMeth
{
	@Inject
	...
	public void toggleNormalPrayer(int style) {
		queueClickPacket();
		queueWidgetActionPacket(1,style,-1,-1);
	}
	...
	public boolean checkReflection() {
		...
		for (Plugin p : pluginManager.getPlugins()) {
			if (p.getClass().getSimpleName().toLowerCase().equals("lavarunecrafterplugin"))
			{
				for (Field f : p.getClass().getDeclaredFields())
				{
					if (f.getName().toLowerCase().equals("widgetpackets")) {
		...
		for (Plugin p : pluginManager.getPlugins()) {
			if (p.getClass().getSimpleName().toLowerCase().equals("packetutilsplugin"))
			{
				for (Field f : p.getClass().getDeclaredFields())
				{
		...
		for (Plugin p : pluginManager.getPlugins()) {
			if (p.getClass().getSimpleName().toLowerCase().equals("ethanapiplugin"))
			{
				apiInstance = p;
